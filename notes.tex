...
We are currently investigating several directions to improve basin correspondence:
\begin{enumerate}
    \item \textbf{Larger basin dimensions}: Testing $d_b \in \{16, 32, 64\}$ with $d_g \in \{16, 32\}$ for total latent dimensions of 300--1,300, using only pairwise exclusivity loss (no entropy/dominance).
    \item \textbf{Unstructured sparse baselines}: Testing whether standard LISTAKM with large latent dimension (512--2048) naturally learns basin-distinguishing sparse supports without explicit partitioning.
    \item \textbf{GenericKM baselines}: Establishing comparison baselines with MLP encoders at various dimensions.
\end{enumerate}

\paragraph{Discussion.}
These results highlight an important tension: while the structured Koopman parameterization \emph{enables} basin-specific dynamics, it does not \emph{guarantee} that the learned representation will discover the correct basin correspondence without explicit supervision. The over-specification finding suggests that the optimization landscape favors basin collapse when the model capacity exactly matches the number of ground-truth basins. Providing excess capacity may smooth the loss landscape and allow gradient-based optimization to find solutions with better basin separation.
