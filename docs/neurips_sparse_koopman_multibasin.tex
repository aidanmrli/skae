...
\begin{abstract}
    ...
    On a \(512\)-dimensional four-well Allen--Cahn system, transferred sparse supports separate held-out basin fates (ARI \(0.704\)) while exact dense collapses to one family, but the sparse map misses a prespecified within-basin uniqueness limit and does not beat dense in PDE forecasting.
...
The independent confirmations use fully observed \(128\)-dimensional Lorenz--96 and a two-channel, periodic \(16\times16\) four-well vector Allen--Cahn system with \(d_x=512\).  Both KAEs use four-times overcomplete lifts in these experiments. Vector-field expressions and more details are in Appendices \ref{app:controlled_inventory}, \ref{app:dysts_inventory_full}, and \ref{app:highdimensional_confirmation}.
...
\caption{Independent high-dimensional evidence; faint lines or paired traces are individual model seeds and opaque marks show means with 95\% model-seed bootstrap intervals. \textbf{(a)} Fixed-data Lorenz--96 forecasting through physical time \(5\). \textbf{(b)} Final-time Allen--Cahn RMSE relative to persistence through physical time \(20\), on a logarithmic ordinate. \textbf{(c)} Final-time test support transfer on Allen--Cahn; basin information denotes \(1-H(B\mid F)/H(B)\). Exact dense transfers one family, hence zero NMI and ARI. The unplotted sparse uniqueness diagnostic \(H(F\mid B)/H(F)=0.396\) misses its prespecified \(0.35\) ceiling.}
...
\paragraph{Transferred supports are basin-informative on a high-dimensional PDE, but not one-to-one.}
On the \(d_x=512,d_z=2{,}048\) four-well Allen--Cahn test, the validation-fitted sparse family map has transfer coverage \(0.988\), ARI \(0.704\) (\([0.629,0.767]\)), NMI \(0.683\), and purity \(0.886\); exact dense transfers one family and has ARI zero. Sparse wins all ten paired ARI comparisons (\(p=0.001953\)). However, its normalized \(H(F\mid B)=0.396\) and mean 10.4 families for four fates fail the frozen uniqueness criterion, so this is bounded basin-information evidence rather than confirmation of one support per basin. At physical time \(20\), direct, dense, and sparse final field MSE are \(0.0366\), \(0.0997\), and \(0.1602\); all beat persistence, but sparse does not beat dense and the direct control is best. Full protocols and conditional-on-one-dataset inference are in \cref{app:highdimensional_confirmation}.
...
The immediate priorities are therefore to reproduce the Allen--Cahn support result with a label-free temporal-consistency objective and a new dataset-level holdout, freeze cadence on validation data with strict finiteness accounting, rerun the staged comparison under one common selector and a newly versioned 512-unique route fit with system-level uncertainty, and test basin-support alignment on additional high-dimensional or empirical multibasin systems.
