...
| --- | --- | --- | --- |
| `absolute:0.001` | Mark a latent coordinate active when its magnitude is above `0.001`. | Direct and easy to interpret; best for asking whether one basin is represented by one exact support away from basin boundaries. | On the selected states far from basin boundaries in the locked final packet, the promoted dense LISTA finalist reaches `H(B\|S)=0.0000`, `H(S\|B)=0.0543`, `U_exact=0.9923`, `H(F\|B)=0.0000`, versus the matched sparse MLP at `H(S\|B)=0.2449` and `U_exact=0.9772`. This is the cleanest exact-support identifiability result. |
