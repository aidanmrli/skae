...
| --- | --- | --- | --- |
| `absolute:0.001` | Mark coordinate `i` active iff `|z_i| > 0.001`. | Direct and easy to interpret; best for asking whether one basin really owns one exact support deep in-basin. | On the selected `deep` slice in the locked final packet, the promoted dense LISTA finalist reaches `H(B\|S)=0.0000`, `H(S\|B)=0.0543`, `U_exact=0.9923`, `H(F\|B)=0.0000`, versus the matched sparse MLP at `H(S\|B)=0.2449` and `U_exact=0.9772`. This is the cleanest exact-support identifiability result. |
| `relative:0.1` | Mark coordinate `i` active iff `|z_i| > 0.1 * max_j |z_j|`. | Scale-relative; still meaningful on dense non-sparse controls where exact zeros are not expected. | Very useful for centered local-law diagnosis, but too fragmented as a deployment router. In the centered-chart packet it gave strong deep local-law wins, but in the self-routed packet the exact-support router was skipped `160/160` times per root on deep `q4`. |
| `topk:8` | Mark exactly the `8` largest-magnitude latent coordinates active. | Fixed support size; much more stable as a routing signal than thresholded exact support. | This is the strongest non-oracle forecasting result. In the self-routed packet, dense LISTA exact-support `topk:8` achieves all-slice `H1000/global` median / win rate `0.228 / 0.920` for `support_gated_k` and `0.275 / 0.947` for `support_local_centered`. The dense zero-sparsity MLP is much weaker at `0.924 / 0.539` and `1.000 / 0.496`. |
...
- `H(B|S)` asks: after I know the support object, how uncertain am I about the
  basin? Low values mean a support object is basin-pure.
...
The cleanest qualitative read is:

- low `H(B\|S)` means a support object is basin-pure,
- low `H(S\|B)` means each basin uses only a few such objects,
- high `U_exact` means one exact support dominates within each basin,
- low `H(F\|B)` means support families align very cleanly to basins.

## Safest Current Claims

1. **Exact-support identifiability claim**

   Safe only in a qualified form: deep inside basins, the promoted dense LISTA
   finalist shows near-unique exact supports on the `absolute:0.001` slice.

2. **Support-family alignment claim**

   This is the safest broad interpretability claim. Family entropy is already
   essentially saturated on the selected deep slice, and family-level basin
   alignment is much more robust than exact-support uniqueness.
...
- Use `absolute:0.001` when making the deep-basin exact-support alignment read.
- Use `topk:8` for the main non-oracle forecasting table and figure.
- Use support families for the broader basin-alignment story and for
