...
forecasting over the reported Dense MLP KAE rows; sparse support families align
with evaluation-only basin labels in the transductive controlled diagnostic and
under frozen transfer on the independent high-dimensional PDE holdout; and a
...
\cref{app:controlled_inventory,app:dysts_inventory_full,app:experimental_details,app:highdimensional_confirmation};
...
\begin{itemize}
  ...
  multibasin.  Allen--Cahn is one synthetic PDE: its primary all-state
  uniqueness score misses the frozen gate by \(0.009\), its exact interior
  slice is evaluation-only, and temporal sparse is worse than dense at long
  forecast horizons.  Neither experiment establishes empirical real-world
  generalization.
\end{itemize}
...
\begin{itemize}
  ...
  \(3\) GPU-hours.
  \item The final Lorenz--96 confirmation uses one two-hour GPU allocation.
