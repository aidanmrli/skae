...
This appendix lists the ten Dysts systems frozen for the corrected
\(dt{\times}30\) forecasting campaign. The systems are drawn from Dysts
...
standardizes coordinates only after trajectories are generated. This roster
does not validate or describe the quarantined historical KAE caches.

\begin{table}[tbp]
\centering
\caption{The ten-system roster and target observation grid for the corrected
Dysts \(dt{\times}30\) campaign. The ``target stored step'' is \(30\) times
the native Dysts integration interval and does not describe the quarantined
historical KAE caches. The final column gives the hand-specified diagnostic
...
\toprule
Display name & Dimension & Native \(dt\) & Target stored step \(30dt\) & Diagnostic count \\
\midrule
...
For the corrected campaign, stored observations are generated from \(\dot x=f_s(x)\)
with a numerically verified observation interval \(\Delta t=30\,dt_{\rm native}\), where
