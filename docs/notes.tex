...
\begin{enumerate}
    \item \textbf{Achieved:} basin-unique sparse supports at realistic basin count (13 basins) with sufficient latent capacity.
    \item \textbf{Achieved:} threshold-free evidence of strong within-basin consistency and between-basin separation.
    \item \textbf{Achieved:} objective-level improvements in separability via block-usage balancing (and selective combined losses).
    \item \textbf{Partially achieved:} stable long-horizon dynamics in selected pairwise regimes (SR $< 1$), with periodic re-encoding as a reliable fallback.
    \item \textbf{Open:} robust label-free assignment from sparse supports to local control models. If sequence training is required, robust stability for non-diagonal Koopman dynamics is still open; if pairwise training is acceptable, this is not the immediate bottleneck.
\end{enumerate}

Overall, the representation side (basin-discriminative sparse coding) is now well supported empirically. The next bottleneck is dynamics-side stabilization under sequence objectives and a fully label-free gating mechanism suitable for downstream control.
