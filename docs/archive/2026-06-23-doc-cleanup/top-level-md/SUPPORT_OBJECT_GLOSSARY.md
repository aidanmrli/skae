...
- exact support, support family, dominant group, and stable support component
  are **support objects** built from those masks, grouped latent structure, or
  empirical support-flow structure.
...
| Stable support component / `C_stab` | A label-free dynamical support object built from the empirical transition graph of high-resolution support states. Support states are merged when they are absorbed into the same recurrent support-flow component. | This is the closest current object to the basin-of-attraction idea: basins are dynamical fate classes, so the support object should be defined by support-flow fate rather than by instantaneous Jaccard overlap alone. | Initial post-hoc results are promising for basin-support alignment but not yet for local-map forecasting. On the first p256 LISTA contrast, `C_stab` gives basin-matched deep-slice objects on `8/8` system-seed pairs, while current `F_abs` does so on `4/8`. A matched four-root encoder comparison separates sparse-support encoders from dense/zero-sparsity MLP: both LISTA roots and sparse MLP reach `8/8` deep-slice count matches with `H(B\|C)=0`, while dense MLP collapses to one component with `H(B\|C)=1.685`. This does not yet show LISTA uniquely beating sparse MLP under `C_stab`. |

## Stable Support Components

`C_stab` is a support object over support trajectories, not a count of
state-space fixed points. Given encoded trajectories, first compute a
high-resolution support state `u_t` from each latent state. In the first
implementation, `u_t` is either an exact `absolute:0.001` support mask or a
high-Jaccard preliminary support family, usually with `base_jaccard=0.8`.
Then build the empirical support-transition graph
...
The graph is filtered by minimum transition count and transition probability.
Strongly connected components that appear in trajectory tails and have low
outbound probability are treated as recurrent support components. For each
observed support state `u`, the algorithm scans future support states in the
same trajectory to estimate an absorption distribution over these recurrent
components. `u` receives a `C_stab` label only when the largest absorption
probability exceeds a confidence threshold; otherwise it is marked uncertain.

This construction is more general than a fixed-point basin counter. A recurrent
support component could represent support behavior near a stable equilibrium,
a limit cycle, a quasiperiodic attractor, a chaotic attractor, or an artifact
of the learned encoder/support threshold. The benchmark basin labels and basin
counts are used only after construction to evaluate whether the discovered
support-fate components align with known basins.
