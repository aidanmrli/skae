...
A new Table 1 in [`docs/neurips_sparse_koopman_multibasin.tex`](neurips_sparse_koopman_multibasin.tex) now reports the same `H(B|F)`, wrong-support-freeze h=1, h=20, and family-count metrics as before, but on the per-basin deep slice. Forecasting columns remain all-held-out rollout MSEs.

The companion entropy strip figure
[`fig_fixed17_entropy_strips_alt2.pdf`](figures/neurips_paper_2026/fig_fixed17_entropy_strips_alt2.pdf)
has also been regenerated from the current-roster per-basin deep rows. The
`|F_abs|` panel uses a linear basin-count scale capped at `6`, with triangles
for larger counts and a `4.20` retained-basin mean reference.

The previous global-slice Table 1 should now be treated as an appendix comparison for the absolute-deep-states reading. The per-basin Table 1 is the reference for the "every basin contributes deep states" mechanistic reading.

May 6 correction: the completed packet below belongs to the earlier root roster.
It did not fully cover the current manuscript Table 1 roster because the current
table adds the plain `p256` LISTA row and uses matched `p256` LISTA-SB instead of
the older `p64` soft-block row. A current-roster rerun has therefore been completed
with output directory name `interpretability_per_basin_deep_current_table1_pass0`.
Those outputs, not the older packet, are used for the main-table replacement.
...
| Queue current Table 1 roster rerun | complete | Submitted via `scripts/queue_per_basin_deep_current_table1_eval.sh`: backfill shards `9477837`--`9477840` with merge `9477841`, matched `p256` LISTA-SB shard `9477842` with merge `9477843`, and plain `p256` LISTA shard `9477844` with merge `9477845`; all completed with exit `0:0`. |
| Build current per-basin Table 1 | complete | Compute job `9479694` wrote `table1_fixed17_alignment.tex`, `table1_fixed17_alignment_per_basin_deep.tex`, CSV, and metadata under `docs/figures/neurips_paper_2026/_tables/`. |
| Regenerate per-basin entropy strip figure | complete | Compute allocation `9480248` ran `tools/make_per_basin_entropy_strips.py` and rewrote `fig_fixed17_entropy_strips_alt2.pdf/png`. |
| Add appendix global-deep comparison | pending | If desired, preserve the old global-slice table as an appendix comparison rather than the main Table 1. |
