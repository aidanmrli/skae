...
unsupervised mixture/local-linear dynamics baselines. It can also drive
label-free local EDMD/Koopman baselines with validation-selected route counts.
These runs do not depend on trained sparse-KAE checkpoints and do not use
basin labels for fitting.
"""
...
LOCAL_EDMD_METHODS: Sequence[str] = (
...
def _build_rows(args: argparse.Namespace) -> List[Dict[str, object]]:
    ...
    for family in baseline_families:
        ...
        elif family == "mixture_local_linear":
            methods = MIXTURE_METHODS
        else:
            methods = LOCAL_EDMD_METHODS
                ...
                rows.append(
                    {
                        ...
                        "local_num_components_grid": args.local_num_components_grid,
                        "local_validation_fraction": args.local_validation_fraction,
                        "local_selection_horizons": args.local_selection_horizons,
                        "local_min_component_transitions": args.local_min_component_transitions,
                        "local_max_abs_state_for_fit": args.local_max_abs_state_for_fit,
                        "env_dt": args.env_dt,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Comma-separated subset of classical_koopman,mixture_local_linear,local_edmd_koopman.",
    )
    ...
    parser.add_argument("--local_num_components_grid", default="1,2,4,8,16")
    parser.add_argument("--local_validation_fraction", type=float, default=0.25)
    parser.add_argument(
        "--local_selection_horizons",
        default="",
        help="Validation horizons for local_edmd_koopman; empty reuses --horizons.",
    )
    parser.add_argument("--local_min_component_transitions", type=int, default=64)
    parser.add_argument("--local_max_abs_state_for_fit", type=float, default=1e6)
            ...
            json.dumps(
                {
                    ...
                    "local_edmd_methods": list(LOCAL_EDMD_METHODS),
                    "config": vars(args),
                },
                indent=2,
            )
