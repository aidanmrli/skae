...
def write_manifest(
    ...
    *,
    ...
    payload = {
        "description": "Ground-truth continuous-time vector fields for the retained 15 multibasin benchmark systems.",
        ...
        "systems": [
            {
                **asdict(field.spec),
                "dt": field.dt,
                "slug": field.spec.slug,
                "log_speed_vmin": field.log_speed_vmin,
                "log_speed_vmax": field.log_speed_vmax,
                "centers": None if field.centers is None else field.centers.round(8).tolist(),
                "individual": individual_paths.get(field.spec.system_key, []),
            }
            for field in fields
        ],
    }
