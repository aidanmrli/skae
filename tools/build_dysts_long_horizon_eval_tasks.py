...
DEFAULT_ROOT_SPECS: Sequence[RootSpec] = (
    RootSpec(
        label="generic_sparse_ns200k_best",
        display_name="sparse MLP",
        model_family="mlp",
        root_dir=Path(
            "/network/scratch/l/lia/skae/paper_followup_recipes_200k_20260309/"
            "paper_followup_recipes/generic_sparse_ns200k_best"
        ),
    ),
    RootSpec(
        label="generic_sparse_sc0_ns200k_best",
        display_name="zero-sparse MLP",
        model_family="mlp",
        root_dir=Path(
            "/network/scratch/l/lia/skae/paper_zero_sparse_benchmark_200k_20260321/"
            "paper_zero_sparse_benchmark/generic_sparse_sc0_ns200k_best"
        ),
    ),
    RootSpec(
        label="lista_dense_promoted_stage4",
    ...
    RootSpec(
        label="lista_blockdiag_ns200k_denseopt_sc3em3",
    ...
    RootSpec(
        label="lista_blockdiag_ns200k_denseopt_sc6em3",
...
def _read_root_specs(path: Path | None) -> List[RootSpec]:
    if path is None:
        return list(DEFAULT_ROOT_SPECS)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--root_specs_tsv", default=None, help="Optional custom root-spec TSV.")
...
def main() -> None:
    args = parse_args()
    root_specs = _read_root_specs(Path(args.root_specs_tsv) if args.root_specs_tsv else None)
    systems = (
