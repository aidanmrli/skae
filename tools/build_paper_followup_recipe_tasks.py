#!/usr/bin/env python3
"""Build task tables for paper-benchmark follow-up recipe reruns."""
    ...
    for item in _parse_csv_list(raw):
        ...
        if len(parts) != 9:
            raise ValueError(
                ...
                "'label:base_variant:num_steps:lr:k_matrix_lr:weight_decay:reconst_coeff:pred_coeff:sparsity_coeff'."
            )
        (
            label,
            base_variant,
        ...
        specs.append(
            {
                ...
                "lr": _parse_optional_float(lr_raw),
                "k_matrix_lr": _parse_optional_float(k_lr_raw),
                "weight_decay": _parse_optional_float(weight_decay_raw),
...
def _build_rows(args: argparse.Namespace) -> tuple[List[Dict[str, object]], List[Dict[str, object]]]:
    systems = _parse_csv_list(args.systems_csv) or [
        spec.system_key for spec in paper_benchmark_systems()
    ]
    ...
    for recipe in recipe_specs:
        model_spec = get_paper_benchmark_model(str(recipe["base_variant"]))
                ...
                rows.append(
                    {
                        ...
                        "lr": recipe["lr"] if recipe["lr"] is not None else "",
                        "k_matrix_lr": recipe["k_matrix_lr"] if recipe["k_matrix_lr"] is not None else "",
                        "weight_decay": recipe["weight_decay"] if recipe["weight_decay"] is not None else "",
                        "env_dt": env_dt,
...
def _manifest_payload(
    *,
    ...
    return {
        "experiment": "paper_followup_recipe_rerun",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build paper-benchmark follow-up recipe tasks.")
    ...
    parser.add_argument("--phase_label", default="paper_followup_recipes", help="Phase label embedded in task rows.")
...
def main() -> None:
    ...
    if args.output_manifest_json:
        payload = _manifest_payload(
            ...
            systems=_parse_csv_list(args.systems_csv) or [spec.system_key for spec in paper_benchmark_systems()],
            seeds=_parse_int_csv(args.seeds_csv, [int(seed) for seed in PAPER_BENCHMARK_SEEDS]),
    ...
    print(f"Wrote {len(rows)} paper follow-up recipe tasks to {output_tsv}")
