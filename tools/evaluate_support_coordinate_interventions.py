...
REDUCER = _load_module(
    ...
    "reduce_transition_rich_interpretability_metrics_coordinate_interventions",
)
...
def _latest_spec(rows_csv: Path, root_label: str, system: str, seed: int):
    specs = REDUCER._load_latest_specs(
        rows_csv,
...
def _label_states(env, states: torch.Tensor, centers: torch.Tensor) -> torch.Tensor:
    ...
    return REDUCER._assign_nearest_centers(states, centers)
...
def main() -> None:
    ...
    basin_labels_t, centers, label_source = REDUCER._label_sequences_and_centers(
        env,
        trajectories,
    ...
    subset_masks = REDUCER._margin_subsets(
        trajectories,
        centers,
        basin_labels=basin_labels if args.depth_slice_mode == "per_basin" else None,
    ...
    z0_np = z0.cpu().numpy()
    active_mask = REDUCER._support_mask(z0_np, scheme=support_scheme, value=support_value)
