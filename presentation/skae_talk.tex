...
% figure caption: centered, small gray, pulled slightly closer to the figure above
\newcommand{\figcap}[1]{\vspace{-0.35em}\par{\small\color{skaegray}\centering #1\par}}
...
\begin{frame}{Koopman operator theory}
  \begin{center}
  The \emphp{Koopman operator} $\mathcal{K}$ acts \emph{linearly} on scalar observables $g:\mathcal{X}\to\R$:
  \[
...
%--- (removed) "Coexisting attractors partition state space": redundant with the
%    Duffing basins slide above, and the same fig1_top panels reappear in
%    "Result 3: unsupervised basin recovery". Restore this frame if you want the
%    procedural benchmark shown up front in the Problem section.

%--- the wall ---
\begin{frame}{No single global linearization}
  \vfill
  \makebox[\linewidth][c]{%
  ...
  \begin{center}\small\color{skaegray}
  Lan \& Mezi\'c 2013 \textperiodcentered\ Brunton et al.\ 2016 \textperiodcentered\ Fathi et al.\ 2024.
  \textperiodcentered\ Pan \& Duraisamy 2024 \textperiodcentered\ Liu et al.\ 2025.\\
  \end{center}
  \vfill
\end{frame}
...
\begin{frame}{Reframing the objective}
  \statement{Don't force all basins through a \emph{single} global linearization.\\[0.6em]
  \emphp{Find distinct \emph{local} linearizations.}}
\end{frame}
...
\begin{frame}{A sparse code encodes both quantities}
  \vspace{0.2em}
  \begin{center}\normalsize
  the \emphp{support} (which coordinates are active) $\;\Rightarrow\;$ \emph{which} regime\\[0.2em]
  the \empht{nonzero values} $\;\Rightarrow\;$ local Koopman coordinates
  \end{center}
  \vspace{0.2em}
  \begin{center}
  \begin{tikzpicture}[font=\scriptsize,>=Stealth,scale=0.95,
      cell/.style={draw=skaeink!60,minimum width=0.34cm,minimum height=0.34cm,inner sep=0}]
    % ---- state space (left): three-region partition ----
    \coordinate (C) at (-4.0,-0.15);
    \fill[skaepurple!28] (C) -- (-5.5,1.6) -- (-2.5,1.6) -- cycle;
    \fill[skaeteal!28]   (C) -- (-5.5,1.6) -- (-5.5,-1.6) -- (-4.0,-1.6) -- cycle;
    \fill[skaeorange!28] (C) -- (-4.0,-1.6) -- (-2.5,-1.6) -- (-2.5,1.6) -- cycle;
    \draw[skaeink!55] (-5.5,-1.6) rectangle (-2.5,1.6);
    \draw[skaeink!35] (C)--(-5.5,1.6); \draw[skaeink!35] (C)--(-2.5,1.6); \draw[skaeink!35] (C)--(-4.0,-1.6);
    \fill[skaepurple] (-4.0,0.85) circle (2.4pt);
    \fill[skaeteal]   (-4.8,-0.65) circle (2.4pt);
    \fill[skaeorange] (-3.2,-0.65) circle (2.4pt);
    \node[skaeink] at (-4.0,-2.05) {state space};
    % ---- encode arrow ----
    \draw[->,skaeblue,very thick] (-2.3,0) -- (0.35,0) node[midway,above,black]{encode $E_\theta$};
    % ---- three sparse code vectors (right), color-matched to each region ----
    \foreach \y/\c in {0.9/skaepurple,0.54/skaepurple,0.18/white,-0.18/white,-0.54/white,-0.9/white}{\node[cell,fill=\c] at (1.2,\y){};}
    \node[skaepurple] at (1.2,-1.35) {$S_1$};
    \foreach \y/\c in {0.9/white,0.54/white,0.18/skaeteal,-0.18/skaeteal,-0.54/white,-0.9/white}{\node[cell,fill=\c] at (2.3,\y){};}
    \node[skaeteal] at (2.3,-1.35) {$S_2$};
    \foreach \y/\c in {0.9/white,0.54/white,0.18/white,-0.18/white,-0.54/skaeorange,-0.9/skaeorange}{\node[cell,fill=\c] at (3.4,\y){};}
    \node[skaeorange] at (3.4,-1.35) {$S_3$};
    \node[skaeink] at (2.3,1.4) {sparse codes $z_k$};
  \end{tikzpicture}
  \end{center}
  \figcap{Each region of state space activates its own \emph{support}: the colored (nonzero) cells; the code is zero elsewhere. Nearby states share a support, giving a union-of-subspaces structure.}
\end{frame}
...
\begin{frame}{Learned ISTA (LISTA): sparse coding as an encoder}
  ...
  \vspace{0.2em}
  \[
    u^{(0)}=\mathrm{shrink}(c,\tau),\ \ u^{(q+1)}=\mathrm{shrink}\!\big(S\,u^{(q)}+c,\tau\big),\ \ z=u^{(Q)}=E_\theta(x)
  \]
...
%--- ("Three structures for the transition K" moved to backup) ---

%--- encoders compared ---
\begin{frame}{Encoders compared}
  \vfill
  \begin{points}
    \item \emphp{LISTA:} sparse by construction (thresholding gives exact zeros)
    \item \empht{Sparse MLP:} induced sparsity (ReLU with an $\ell_1$ penalty)
    \item \emphc{Dense MLP:} no sparsity mechanism (control baseline)
  \end{points}
  \vfill
\end{frame}
...
%--- support objects (compressed: exact support + families) ---
\begin{frame}{Support objects}
  \vfill
  \begin{center}\normalsize
  \emphp{Exact support:}\ \ $S_{\mathrm{abs}}(x)=\{\,i:|z_i|>10^{-3}\,\}$,\ \ the set of active coordinates.
  \end{center}
  \vspace{0.7em}
  \begin{center}\normalsize
  \empht{Support families} group nearby exact supports by \emph{Jaccard overlap}\\ (intersection over union), merging when $J\ge 0.5$:
  \end{center}
  \vspace{0.2em}
  \[
    J(m,r)=\frac{|\,\mathrm{active}(m)\cap\mathrm{active}(r)\,|}{|\,\mathrm{active}(m)\cup\mathrm{active}(r)\,|}
  \]
  \vspace{0.3em}
  \begin{center}\small\color{skaegray}
...
%--- ("Two evaluation diagnostics" moved to backup) ---
...
\begin{frame}{Benchmarks: 25 systems}
  ...
  \begin{points}
    ...
    \item \textbf{10 chaotic Dysts flows}
    \item[] \hspace{1em}{\normalsize\color{skaegray} Chua, Shimizu--Morioka, and more: more challenging flows}
  \end{points}
  \vfill
\end{frame}
...
\begin{frame}{Experimental questions}
  \vfill
  \begin{points}
    \item[\textbf{1.}] Do sparse encoders lead to better \emph{forecasting}?
...
\begin{frame}{Result 1: sparse encoders forecast better}
  \begin{center}
    \makebox[\linewidth][c]{%
      \includegraphics[width=0.475\linewidth,trim=0 0 336 0,clip]{figs/fig_forecasting_horizon_trends.pdf}%
      \hspace{0.025\linewidth}%
      \includegraphics[width=0.475\linewidth]{figs/fig2b_dysts_no_lista_sb.pdf}%
    }
  \end{center}
  \figcap{Sparse models improve on the dense baseline at \emph{every} horizon; at $H\!=\!1000$ the dense baseline incurs \emphc{17--27$\times$} higher error.}
\end{frame}
...
\begin{frame}{Result 1: qualitative behavior on chaotic flows}
  ...
  \end{center}
  \figcap{The \emphc{dense MLP (red)} forecasting is much qualitatively worse than the \emphp{LISTA (blue)} forecast. \textbf{Sparsity improves long-horizon forecasts.}}
\end{frame}
...
\begin{frame}{Result 2: are the supports functionally necessary?}
  ...
  \vspace{0.4em}
  \begin{center}
    \includegraphics[width=0.96\linewidth]{figs/fig3_interventions_full.png}
  \end{center}
  \figcap{Zeroing the top-$k$ active coefficients sharply increases error even at short horizons (left); random reassignment to inactive coordinates is substantially more damaging (right).}
\end{frame}
...
\begin{frame}{Result 2: trajectory-level effect}
  ...
  \end{center}
  \figcap{Randomly reassigning the active coordinates (red) drives trajectories away from the true dynamics; standard rollouts (black) remain accurate.}
\end{frame}
...
\begin{frame}{Result 3: unsupervised basin recovery}
  ...
  \end{center}
  \figcap{LISTA support families partition the flows; coloured = support/basin \emph{matches}. LISTA exposes $\approx$\,4.0 families vs.\ the benchmark mean/median 4.20/4; \emphc{dense MLP} collapses to 1. No basin labels are used during training.}
\end{frame}
...
\begin{frame}{(WIP) Future work: toward structured world models}
  \begin{center}\normalsize
...
%------------------ BACKUP: transition structures for K ------------------
\begin{frame}{Backup: three structures for the transition $K$}
  \vspace{0.6em}
  \begin{columns}[T]
...
%------------------ BACKUP: evaluation diagnostics ------------------
\begin{frame}{Backup: two evaluation diagnostics}
  \vfill
  \begin{points}
