...
ROOT_SPECS_TSV="${TASK_DIR}/dysts_long_horizon_root_specs.tsv"
...
uv run python tools/build_dysts_long_horizon_eval_tasks.py \
  --output_tsv "${TASK_TSV}" \
  --output_summary_json "${TASK_SUMMARY_JSON}" \
  --output_root_specs_tsv "${ROOT_SPECS_TSV}" \
...
VALIDATE_JOB_ID=$(
  ...
  sbatch --parsable -p long --dependency=afterany:${CACHE_JOB_ID} --array=${VALIDATION_INDEX}-${VALIDATION_INDEX}%1 scripts/run_dysts_long_horizon_eval_array.sh
)
...
EVAL_JOB_ID=$(
  ...
  DYSTS_CACHE_NUM_WORKERS="${DYSTS_CACHE_NUM_WORKERS}" \
  BATCH_SIZE="${BATCH_SIZE}" \
  sbatch --parsable -p long --dependency=afterok:${VALIDATE_JOB_ID} --array=0-$((TASK_COUNT - 1))%${ARRAY_PARALLEL} scripts/run_dysts_long_horizon_eval_array.sh
)
