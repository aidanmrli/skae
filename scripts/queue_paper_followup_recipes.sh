...
dense_label = sys.argv[6]
dense_path = sys.argv[7]
...
print(",".join(labels))
' "${TASK_TSV}" "${ROOT_SPECS_FILE}" "${BASE_OUT}" "${PHASE_LABEL}" "${ANCHOR_PATH}" "${DENSE_PROMOTED_LABEL}" "${DENSE_PROMOTED_PATH}" > "${ROOT_SPEC_DIR}/candidate_roots.csv"

CANDIDATE_ROOTS_CSV="$(tr -d '\n' < "${ROOT_SPEC_DIR}/candidate_roots.csv")"
GENERIC_200K_LABEL="$(echo "${CANDIDATE_ROOTS_CSV}" | tr ',' '\n' | grep '^generic_sparse' | head -n 1)"
BLOCKDIAG_CANDIDATES_CSV="$(echo "${CANDIDATE_ROOTS_CSV}" | tr ',' '\n' | grep '^lista_blockdiag' | paste -sd, -)"
GENERIC_200K_AND_BLOCKDIAG_CANDIDATES_CSV="${CANDIDATE_ROOTS_CSV},${DENSE_PROMOTED_LABEL}"
DENSE_COMPARISON_CANDIDATES_CSV="${GENERIC_200K_LABEL}"
if [[ -n "${BLOCKDIAG_CANDIDATES_CSV}" ]]; then
  DENSE_COMPARISON_CANDIDATES_CSV="${DENSE_COMPARISON_CANDIDATES_CSV},${BLOCKDIAG_CANDIDATES_CSV}"
  GENERIC_200K_COMPARISON_CANDIDATES_CSV="${BLOCKDIAG_CANDIDATES_CSV},${DENSE_PROMOTED_LABEL}"
else
...
COMPARE_GENERIC_200K_JOB_ID=$(ROWS_CSV="${COLLECT_DIR}/forecasting_rows.csv" OUT_DIR="${COMPARE_DIR}/vs_${GENERIC_200K_LABEL}" CANDIDATE_ROOTS_CSV="${GENERIC_200K_COMPARISON_CANDIDATES_CSV}" ANCHOR_ROOT="${GENERIC_200K_LABEL}" HORIZON=1000 sbatch --dependency=afterany:${COLLECT_JOB_ID} scripts/compare_paper_benchmark.sh | awk '{print $4}')
...
echo "Compare vs ${GENERIC_200K_LABEL} job: ${COMPARE_GENERIC_200K_JOB_ID}"
