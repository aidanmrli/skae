...
EXPERIMENT_TAG="${EXPERIMENT_TAG:-dense_lista_recipe_validation_${DATE_TAG}}"
PHASE_LABEL="${PHASE_LABEL:-recipe_validation}"
...
SYSTEMS_CSV="${SYSTEMS_CSV:-blended,competitive_lv,duffing,dysts:Dadras,dysts:Hadley,dysts:LuChenCheng,dysts:SanUmSrisuchinwong,multiwell_gradient}"
RECIPE_SPECS_CSV="${RECIPE_SPECS_CSV:-lista_dense_ns200k_lr5em5_klr5em6_wd1em4_rc3em2_pc1ep0_sc3em3:200000:5e-5:5e-6:1e-4:0.03:1.0:0.003,lista_dense_ns100k_lr5em5_klr5em6_wd1em4_rc3em2_pc1ep0_sc1em2:100000:5e-5:5e-6:1e-4:0.03:1.0:0.012}"

mkdir -p "${TASK_DIR}" "${ROOT_SPEC_DIR}" "${COLLECT_DIR}" "${COMPARE_DIR}"

TASK_TSV="${TASK_DIR}/dense_lista_recipe_validation.tsv"
MANIFEST_JSON="${TASK_DIR}/dense_lista_recipe_validation_manifest.json"
ROOT_SPECS_FILE="${ROOT_SPEC_DIR}/dense_lista_recipe_validation_roots.txt"
...
uv run python tools/build_dense_lista_recipe_tasks.py "${BUILD_ARGS[@]}"
...
echo "Queued dense LISTA recipe validation."
