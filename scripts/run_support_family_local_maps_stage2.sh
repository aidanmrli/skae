...
SAVE_TRAIN_CHECKPOINT="${SAVE_TRAIN_CHECKPOINT:-0}"
...
if [[ "${SAVE_METRICS_HISTORY}" == "1" ]]; then
  CMD+=(--save_metrics_history)
fi
if [[ "${SAVE_TRAIN_CHECKPOINT}" == "1" ]]; then
  CMD+=(--save_train_checkpoint)
fi
if [[ "${SAVE_STAGE2_ARTIFACTS}" == "1" ]]; then
  CMD+=(--save_stage2_artifacts)
fi
