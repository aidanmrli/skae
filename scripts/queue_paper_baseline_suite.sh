...
#   SEEDS_CSV=0,1,2
#   BASELINE_FAMILIES=classical_koopman,mixture_local_linear,local_edmd_koopman
...
LOCAL_NUM_COMPONENTS_GRID="${LOCAL_NUM_COMPONENTS_GRID:-1,2,4,8,16}"
LOCAL_VALIDATION_FRACTION="${LOCAL_VALIDATION_FRACTION:-0.25}"
LOCAL_SELECTION_HORIZONS="${LOCAL_SELECTION_HORIZONS:-}"
LOCAL_MIN_COMPONENT_TRANSITIONS="${LOCAL_MIN_COMPONENT_TRANSITIONS:-64}"
LOCAL_MAX_ABS_STATE_FOR_FIT="${LOCAL_MAX_ABS_STATE_FOR_FIT:-1e6}"
ENV_DT="${ENV_DT:-0.0}"
...
uv run python tools/build_paper_baseline_tasks.py \
  ...
  --local_num_components_grid "${LOCAL_NUM_COMPONENTS_GRID}" \
  --local_validation_fraction "${LOCAL_VALIDATION_FRACTION}" \
  --local_selection_horizons "${LOCAL_SELECTION_HORIZONS}" \
  --local_min_component_transitions "${LOCAL_MIN_COMPONENT_TRANSITIONS}" \
  --local_max_abs_state_for_fit "${LOCAL_MAX_ABS_STATE_FOR_FIT}" \
  --env_dt "${ENV_DT}" \
