#!/bin/bash
#
# Run controlled-transfer support-refresh evaluation without local predictors.
#
# Required env vars:
...
#   INCLUDE_NO_TRANSFER_CONTROLS=1
#   DEVICE=cpu
#   SMOKE=0
#
#SBATCH --job-name=tr_sup_refresh
#SBATCH --ntasks=1
#SBATCH --partition=long
#SBATCH --cpus-per-task=4
#SBATCH --mem=24G
#SBATCH --time=12:00:00
#SBATCH -o /network/scratch/l/lia/skae/tr-sup-refresh-%A.out
#SBATCH -e /network/scratch/l/lia/skae/tr-sup-refresh-%A.err
...
SEEDS_CSV="${SEEDS_CSV:-}"
SUPPORT_DEFINITIONS="${SUPPORT_DEFINITIONS:-topk:8}"
...
REENCODE_PERIODS="${REENCODE_PERIODS:-1,10}"
...
echo "============================================="
echo "Transition-Rich Controlled Support Refresh"
...
echo "INCLUDE_NO_TRANSFER_CONTROLS: ${INCLUDE_NO_TRANSFER_CONTROLS}"
echo "DEVICE: ${DEVICE}"
echo "SMOKE: ${SMOKE}"
echo "============================================="

uv run python tools/evaluate_transition_rich_controlled_support_refresh.py \
  --rows_csvs "${ROWS_CSVS}" \
  --output_dir "${OUT_DIR}" \
