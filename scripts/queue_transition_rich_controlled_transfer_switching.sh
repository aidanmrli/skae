#!/bin/bash
#
# Submit controlled-transfer support-switching shards.
#
# Required env vars:
...
for root_label in "${ROOT_LABELS[@]}"; do
  ...
  for seed_split in "${SEED_SPLITS[@]}"; do
    ...
    job_id=$(
      ...
      SMOKE="${SMOKE:-0}" \
        sbatch \
          --job-name="tr_ctrl_${shard_slug}" \
          --time="${WALLTIME}" \
          --cpus-per-task="${CPUS}" \
          --mem="${MEM}" \
          --output="${LOG_DIR}/${shard_slug}-%A.out" \
          --error="${LOG_DIR}/${shard_slug}-%A.err" \
          scripts/run_transition_rich_controlled_transfer_switching.sh | awk '{print $4}'
    )
    JOB_IDS+=("${job_id}")
  done
done
...
printf 'CONTROLLED_TRANSFER_JOB_IDS_CSV=%q\n' "$(IFS=,; echo "${JOB_IDS[*]}")"
printf 'LOG_DIR=%q\n' "${LOG_DIR}"
printf 'SHARDS_DIR=%q\n' "${OUT_DIR}/shards"
