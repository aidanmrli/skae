#!/bin/bash
#
# Simple-environment forecasting benchmark: LISTA "best" carry-over with sequence loss (L=8).
# Grid: 13 systems x 3 seeds = 39 runs.
#
# Submit:
#   sbatch scripts/sweep_simple_envs_seq8_lista_best.sh
#
# Optional overrides:
#   BASE_OUT=/network/scratch/l/lia/skae/simple_envs_seq8/lista_best
#   NUM_STEPS=10000 BATCH_SIZE=256 TARGET_SIZE=256
#   RECONST_COEFF=0.5 PRED_COEFF=1.0 SPARSITY_COEFF=1.5
#   LISTA_ALPHA=0.35 LISTA_NUM_LOOPS=5 LISTA_FINAL_OP=relu
#   K_STRUCTURE=block_diagonal K_BLOCK_SIZE=32
#   SEQUENCE_LENGTH=8 EVAL_PROFILE=full
#
# NOTE:
#   This script intentionally avoids observation/action normalization flags.
#
#SBATCH --job-name=simple_lista_s8
#SBATCH --ntasks=1
...
#SBATCH -o /network/scratch/l/lia/skae/simple-lista-s8-%A_%a.out
#SBATCH --requeue
#SBATCH --array=0-38
...
BASE_OUT="${BASE_OUT:-/network/scratch/l/lia/skae/simple_envs_seq8/lista_best}"
mkdir -p "${BASE_OUT}"
...
SPARSITY_COEFF="${SPARSITY_COEFF:-1.5}"
...
echo "============================================="
echo "Simple Envs Sequence-L8 | LISTA Best"
