#!/bin/bash
#
# Submit controlled-transfer support-refresh shards.
#
# Required env vars:
...
for root_label in "${ROOT_LABELS[@]}"; do
  ...
  for seed_split in "${SEED_SPLITS[@]}"; do
    ...
    job_id=$(
      ...
      SMOKE="${SMOKE:-0}" \
        sbatch \
          --job-name="tr_sup_refresh_${shard_slug}" \
          --time="${WALLTIME}" \
          --cpus-per-task="${CPUS}" \
          --mem="${MEM}" \
          --output="${LOG_DIR}/${shard_slug}-%A.out" \
          --error="${LOG_DIR}/${shard_slug}-%A.err" \
          scripts/run_transition_rich_controlled_support_refresh.sh | awk '{print $4}'
    )
    JOB_IDS+=("${job_id}")
  done
done
...
MERGE_JOB_ID=$(
  ...
  SEEDS_CSV="${SEEDS_CSV}" \
    sbatch \
      ...
      scripts/merge_transition_rich_controlled_support_refresh_shards.sh | awk '{print $4}'
)
...
printf 'CONTROLLED_SUPPORT_REFRESH_JOB_IDS_CSV=%q\n' "$(IFS=,; echo "${JOB_IDS[*]}")"
printf 'CONTROLLED_SUPPORT_REFRESH_MERGE_JOB_ID=%q\n' "${MERGE_JOB_ID}"
printf 'LOG_DIR=%q\n' "${LOG_DIR}"
printf 'SHARDS_DIR=%q\n' "${OUT_DIR}/shards"
printf 'MERGED_DIR=%q\n' "${OUT_DIR}/merged"
