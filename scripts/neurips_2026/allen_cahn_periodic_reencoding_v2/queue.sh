...
#SBATCH --job-name=ac-periodic-v2-queue
#SBATCH --output=/network/scratch/l/lia/skae/ac-periodic-v2-queue-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-periodic-v2-queue-%j.err
...
PACKAGE_DIR="${PROJECT_DIR}/experiments/neurips_2026/allen_cahn_periodic_reencoding_v2"
SCRIPT_DIR="${PROJECT_DIR}/scripts/neurips_2026/allen_cahn_periodic_reencoding_v2"
CARD_PATH="${PACKAGE_DIR}/prediction_card.json"
SOURCE_MANIFEST="${PACKAGE_DIR}/source_manifest.sha256"
OUTPUT_ROOT="/network/scratch/l/lia/skae/allen_cahn_periodic_reencoding_confirmation_20260721_v2"
SMOKE_RECEIPT="/network/scratch/l/lia/skae/allen_cahn_periodic_reencoding_smoke_20260721_v2/smoke_receipt.json"
UUID_PROBE="/network/scratch/l/lia/skae/allen_cahn_periodic_reencoding_smoke_20260721_v2/lineage_uuid_probe.json"
...
AUTHORIZATION="${SKAE_PERIODIC_REENCODING_V2_AUTHORIZE:-}"
EXPECTED_AUTHORIZATION="YES_I_AUTHORIZE_FROZEN_PERIODIC_REENCODING_V2"
...
if [[ ! -f "${UUID_PROBE}" ]] || \
   [[ "$(jq -r '.status' "${UUID_PROBE}")" != "passed_real_cuda_uuid_strict_json" ]] || \
   [[ "$(jq -r '.card_sha256' "${UUID_PROBE}")" != "${EXPECTED_CARD_SHA256}" ]] || \
   [[ "$(jq -r '.source_manifest_sha256' "${UUID_PROBE}")" != "${EXPECTED_SOURCE_MANIFEST_SHA256}" ]] || \
   [[ "$(jq -r '.slurm_job_id' "${UUID_PROBE}")" != "$(jq -r '.slurm_job_id' "${SMOKE_RECEIPT}")" ]] || \
   [[ "$(jq -r '.raw_uuid_type' "${UUID_PROBE}")" != "_CUuuid" ]] || \
   [[ ! "$(jq -r '.gpu_uuid' "${UUID_PROBE}")" =~ ^GPU-.+ ]] || \
   [[ "$(jq -r '.scientific_outcomes_accessed' "${UUID_PROBE}")" != "false" ]]; then
  echo "A current real-CUDA strict-JSON UUID probe is required." >&2
  exit 1
fi
...
EXPORTS="ALL,EXPECTED_CARD_SHA256=${EXPECTED_CARD_SHA256},EXPECTED_SOURCE_MANIFEST_SHA256=${EXPECTED_SOURCE_MANIFEST_SHA256},EXPECTED_SMOKE_RECEIPT_SHA256=${EXPECTED_SMOKE_RECEIPT_SHA256},SKAE_PERIODIC_REENCODING_V2_AUTHORIZE=${AUTHORIZATION}"
