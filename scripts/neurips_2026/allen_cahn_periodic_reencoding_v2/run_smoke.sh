#!/usr/bin/env bash
#SBATCH --job-name=ac-periodic-v2-smoke
#SBATCH --output=/network/scratch/l/lia/skae/ac-periodic-v2-smoke-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-periodic-v2-smoke-%j.err
...
OUTPUT_ROOT="/network/scratch/l/lia/skae/allen_cahn_periodic_reencoding_smoke_20260721_v2"
...
AUTHORIZATION="${SKAE_PERIODIC_V2_SMOKE_AUTHORIZE:-}"

if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  echo "Submit this outcome-free smoke with sbatch." >&2
  exit 1
fi
if [[ "${AUTHORIZATION}" != "YES_I_AUTHORIZE_OUTCOME_FREE_PERIODIC_SMOKE_V2" ]]; then
...
RAW_TELEMETRY="$(mktemp /network/scratch/l/lia/skae/ac-periodic-v2-smoke-${SLURM_JOB_ID}-XXXXXX.csv)"
nvidia-smi \
...
uv run python -m \
  experiments.neurips_2026.allen_cahn_periodic_reencoding_v2.uuid_probe \
  --card "${CARD_PATH}" \
  --expected-card-sha256 "${EXPECTED_CARD_SHA256}" \
  --source-manifest "${SOURCE_MANIFEST}" \
  --expected-source-manifest-sha256 "${EXPECTED_SOURCE_MANIFEST_SHA256}" \
