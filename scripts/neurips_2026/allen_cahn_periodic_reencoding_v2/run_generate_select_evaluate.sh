#!/usr/bin/env bash
#SBATCH --job-name=ac-periodic-v2-gpu
#SBATCH --output=/network/scratch/l/lia/skae/ac-periodic-v2-gpu-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-periodic-v2-gpu-%j.err
...
if [[ "$(sha256sum "${CARD_PATH}" | awk '{print $1}')" != "${EXPECTED_CARD_SHA256}" ]]; then
  echo "Prediction-card hash differs from the authorized freeze." >&2
  exit 1
fi
if [[ "$(sha256sum "${SOURCE_MANIFEST}" | awk '{print $1}')" != "${EXPECTED_SOURCE_MANIFEST_SHA256}" ]]; then
  echo "Source-manifest hash differs from the authorized freeze." >&2
  exit 1
fi
...
RAW_TELEMETRY="$(mktemp /network/scratch/l/lia/skae/ac-periodic-v2-gpu-${SLURM_JOB_ID}-XXXXXX.csv)"
nvidia-smi \
...
uv run python -m experiments.neurips_2026.allen_cahn_periodic_reencoding_v2.run \
  --card "${CARD_PATH}" \
