...
#SBATCH --job-name=ac-periodic-v5-queue
#SBATCH --output=/network/scratch/l/lia/skae/ac-periodic-v5-queue-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-periodic-v5-queue-%j.err
...
SCRIPT_DIR="${PROJECT_DIR}/scripts/neurips_2026/allen_cahn_periodic_reencoding_v5"
...
CUDA_VISIBLE_DEVICES="" uv run python -m \
  ...
  --source-manifest "${SOURCE_MANIFEST}" \
  --expected-source-manifest-sha256 "${EXPECTED_SOURCE_MANIFEST_SHA256}"
EXPORTS="ALL,EXPECTED_CARD_SHA256=${EXPECTED_CARD_SHA256},EXPECTED_SOURCE_MANIFEST_SHA256=${EXPECTED_SOURCE_MANIFEST_SHA256},EXPECTED_SMOKE_RECEIPT_SHA256=${EXPECTED_SMOKE_RECEIPT_SHA256},EXPECTED_UUID_PROBE_SHA256=${EXPECTED_UUID_PROBE_SHA256},SKAE_PERIODIC_REENCODING_V5_AUTHORIZE=${AUTHORIZATION}"
