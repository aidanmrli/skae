#!/usr/bin/env bash
#SBATCH --job-name=gkrf-telemetry
...
uv run python -m experiments.neurips_2026.global_k_residual_forecast.telemetry \
  --mode "${MODE}" \
  --expected-card-sha256 "${EXPECTED_CARD_SHA256}" \
  --expected-task-sha256 "${EXPECTED_TASK_SHA256}" \
  --expected-source-manifest-sha256 "${EXPECTED_SOURCE_MANIFEST_SHA256}" \
  --output-root "${OUTPUT_ROOT}" \
  --output "${OUTPUT_ROOT}/${MODE}/gpu_assessment.json"
