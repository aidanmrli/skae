#!/usr/bin/env bash
#SBATCH --job-name=ac-fate-extract
#SBATCH --output=/network/scratch/l/lia/skae/ac-fate-extract-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-fate-extract-%j.err
...
if [[ ! -f "${OUTPUT_ROOT}/profile/decision.json" ]]; then
  echo "Authenticated profile is missing." >&2
  exit 1
fi
...
RAW_TELEMETRY="$(mktemp /network/scratch/l/lia/skae/ac-fate-extract-${SLURM_JOB_ID}-XXXXXX.csv)"
...
cd "${PROJECT_DIR}"
uv run python -m experiments.neurips_2026.allen_cahn_early_fate_probe.extract \
...
trap - EXIT
uv run python -m experiments.neurips_2026.allen_cahn_early_fate_probe.telemetry scientific \
  --expected-card-sha256 "${EXPECTED_CARD_SHA256}" \
  --expected-source-manifest-sha256 "${EXPECTED_SOURCE_MANIFEST_SHA256}" \
  --expected-task-manifest-sha256 "${EXPECTED_TASK_MANIFEST_SHA256}" \
  --output-root "${OUTPUT_ROOT}" \
