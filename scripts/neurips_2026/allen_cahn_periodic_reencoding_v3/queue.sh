...
#SBATCH --job-name=ac-periodic-v3-queue
#SBATCH --output=/network/scratch/l/lia/skae/ac-periodic-v3-queue-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-periodic-v3-queue-%j.err
...
SCRIPT_DIR="${PROJECT_DIR}/scripts/neurips_2026/allen_cahn_periodic_reencoding_v3"
...
if [[ ! -f "${UUID_PROBE}" ]] || \
   [[ "$(sha256sum "${UUID_PROBE}" | awk '{print $1}')" != "${EXPECTED_UUID_PROBE_SHA256}" ]]; then
  echo "A hash-bound UUID probe is required." >&2
  exit 1
fi
if ! jq -e \
  ...
  echo "The real-CUDA UUID probe did not pass every cross-source gate." >&2
  exit 1
fi
...
EXPORTS="ALL,EXPECTED_CARD_SHA256=${EXPECTED_CARD_SHA256},EXPECTED_SOURCE_MANIFEST_SHA256=${EXPECTED_SOURCE_MANIFEST_SHA256},EXPECTED_SMOKE_RECEIPT_SHA256=${EXPECTED_SMOKE_RECEIPT_SHA256},EXPECTED_UUID_PROBE_SHA256=${EXPECTED_UUID_PROBE_SHA256},SKAE_PERIODIC_REENCODING_V3_AUTHORIZE=${AUTHORIZATION}"
