#!/usr/bin/env bash
#SBATCH --job-name=ac-periodic-v3-smoke
#SBATCH --output=/network/scratch/l/lia/skae/ac-periodic-v3-smoke-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-periodic-v3-smoke-%j.err
...
OUTPUT_ROOT="/network/scratch/l/lia/skae/allen_cahn_periodic_reencoding_smoke_20260721_v3"
...
AUTHORIZATION="${SKAE_PERIODIC_V3_SMOKE_AUTHORIZE:-}"

if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  echo "Submit this outcome-free smoke with sbatch." >&2
  exit 1
fi
if [[ "${AUTHORIZATION}" != "YES_I_AUTHORIZE_OUTCOME_FREE_PERIODIC_SMOKE_V3" ]]; then
...
RAW_TELEMETRY="$(mktemp /network/scratch/l/lia/skae/ac-periodic-v3-smoke-${SLURM_JOB_ID}-XXXXXX.csv)"
nvidia-smi \
