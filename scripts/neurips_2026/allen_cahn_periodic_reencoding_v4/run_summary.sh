#!/usr/bin/env bash
#SBATCH --job-name=ac-periodic-v4-summary
#SBATCH --output=/network/scratch/l/lia/skae/ac-periodic-v4-summary-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-periodic-v4-summary-%j.err
...
EXPECTED_GUARD_SHA256="$(CUDA_VISIBLE_DEVICES="" uv run python -m \
  experiments.neurips_2026.allen_cahn_periodic_reencoding_v4.guard outcome \
  --card "${CARD_PATH}" \
