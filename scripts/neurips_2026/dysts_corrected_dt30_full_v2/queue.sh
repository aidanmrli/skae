#!/usr/bin/env bash
#SBATCH --job-name=queue-dysts-dt30-v5
...
PACKAGE="experiments/neurips_2026/dysts_corrected_dt30_full_v2"
SOURCE_MANIFEST="${PROJECT_DIR}/${PACKAGE}/source_manifest.sha256"
RESULTS_DIR="${SKAE_SCRATCH_ROOT}/results/dysts_corrected_dt30_full_20260722_v5"
BASE_OUT="${SKAE_SCRATCH_ROOT}/dysts_corrected_dt30_full_20260722_v5"
SMOKE_BASE_OUT="${SKAE_SCRATCH_ROOT}/dysts_corrected_dt30_smoke_20260722_v5"
...
for path in "${RESULTS_DIR}" "${BASE_OUT}" "${SMOKE_BASE_OUT}"; do
  [[ ! -e "${path}" ]] || { echo "Refusing to reuse ${path}" >&2; exit 1; }
done
[[ -d "${CACHE_DIR}" ]] || { echo "Missing corrected cache root ${CACHE_DIR}" >&2; exit 1; }
...
uv run pytest \
  ...
  tests/test_train.py::TestEvaluate \
...
uv run skae-paper tasks dysts \
  ...
  --output_manifest_json "${SMOKE_MANIFEST}" --systems_csv dysts:Chua \
...
uv run skae-paper tasks dysts \
  ...
  --output_manifest_json "${FULL_MANIFEST}" --num_steps 100000 \
  --dt_multiplier 30 --lista_sb_num_loops 1 --dysts_cache_profile full
...
SMOKE_CACHE_JOB=$(SYSTEMS_FILE="${SMOKE_SYSTEMS_FILE}" CACHE_DIR="${CACHE_DIR}" \
  CACHE_NUM_WORKERS=2 PROFILES=smoke SPLITS="train val" DYSTS_DT_MULTIPLIER=30 \
  SOURCE_MANIFEST="${SOURCE_MANIFEST}" \
  sbatch --parsable --array=0-1 scripts/neurips_2026/dysts/prebuild_cache.sh)
SMOKE_TRAIN_JOB=$(TASK_TSV="${SMOKE_TSV}" TASK_TSV_SHA256="${SMOKE_HASH}" \
  BASE_OUT="${SMOKE_BASE_OUT}" DYSTS_CACHE_DIR="${CACHE_DIR}" \
  PACK_SIZE=12 PACK_CONCURRENCY=12 TRAIN_SKIP_EVAL=1 \
  SOURCE_MANIFEST="${SOURCE_MANIFEST}" GPU_TELEMETRY_INTERVAL=30 \
  sbatch --parsable --dependency="afterok:${SMOKE_CACHE_JOB}" --array=0-0 \
    --cpus-per-task=12 --mem=32G --time=02:00:00 \
    scripts/common/run_benchmark_packed_array.sh)
SMOKE_GATE_JOB=$(SOURCE_MANIFEST="${SOURCE_MANIFEST}" \
  SMOKE_BASE_OUT="${SMOKE_BASE_OUT}" SMOKE_GATE="${SMOKE_GATE}" \
  DYSTS_CACHE_DIR="${CACHE_DIR}" \
  sbatch --parsable --dependency="afterok:${SMOKE_TRAIN_JOB}" \
    scripts/neurips_2026/dysts_corrected_dt30_full_v2/adjudicate_smoke.sh)
FULL_CACHE_JOB=$(SYSTEMS_FILE="${SYSTEMS_FILE}" CACHE_DIR="${CACHE_DIR}" \
  CACHE_NUM_WORKERS=2 PROFILES=full SPLITS="train val policy test" \
...
FULL_TRAIN_JOB=$(TASK_TSV="${FULL_TSV}" TASK_TSV_SHA256="${FULL_HASH}" \
  BASE_OUT="${BASE_OUT}" DYSTS_CACHE_DIR="${CACHE_DIR}" \
  PACK_SIZE=12 PACK_CONCURRENCY=12 TRAIN_SKIP_EVAL=1 \
  SOURCE_MANIFEST="${SOURCE_MANIFEST}" GPU_TELEMETRY_INTERVAL=60 \
  sbatch --parsable --dependency="afterok:${FULL_CACHE_JOB}" --array=0-74%24 \
    --cpus-per-task=12 --mem=32G --time=3-00:00:00 \
    scripts/common/run_benchmark_packed_array.sh)
EVAL_QUEUE_JOB=$(RESULTS_DIR="${EVAL_DIR}" INPUT_ROOT_SPECS_TSV="${ROOT_SPECS}" \
  ...
  REQUIRE_COMPLETE_COVERAGE=1 EXPECTED_TASK_COUNT=900 \
