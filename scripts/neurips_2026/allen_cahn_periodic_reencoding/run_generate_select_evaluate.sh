#!/usr/bin/env bash
#SBATCH --job-name=ac-periodic-gpu
#SBATCH --output=/network/scratch/l/lia/skae/ac-periodic-gpu-%j.out
#SBATCH --error=/network/scratch/l/lia/skae/ac-periodic-gpu-%j.err
...
RAW_TELEMETRY="$(mktemp /network/scratch/l/lia/skae/ac-periodic-gpu-${SLURM_JOB_ID}-XXXXXX.csv)"
nvidia-smi \
...
uv run python -m experiments.neurips_2026.allen_cahn_periodic_reencoding.run \
  --card "${CARD_PATH}" \
