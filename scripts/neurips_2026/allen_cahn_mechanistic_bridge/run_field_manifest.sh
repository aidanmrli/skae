#!/usr/bin/env bash
#SBATCH --job-name=ac-bridge-field-freeze
...
cd "${REPO_ROOT}"
uv run python -m experiments.neurips_2026.allen_cahn_mechanistic_bridge.freeze_field_artifacts \
  --artifact_root "${OUTPUT_ROOT}" --telemetry_root "${OUTPUT_ROOT}/telemetry" \
  --output "${OUTPUT_ROOT}/field_manifest.json" --decision "${MECHANISM_DECISION}" \
