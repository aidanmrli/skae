...
EXPECTED_FIELD_MANIFEST_SHA256=__FREEZE_AFTER_FIELD_STAGE__
...
test "$(sha256sum "${OUTPUT_ROOT}/dataset_manifest.json" | awk '{print $1}')" = "${EXPECTED_DATASET_MANIFEST_SHA256}"
test "$(sha256sum "${OUTPUT_ROOT}/field_manifest.json" | awk '{print $1}')" = "${EXPECTED_FIELD_MANIFEST_SHA256}"
test ! -e "${OUTPUT_ROOT}/summary"
sbatch --parsable --export=ALL,OUTPUT_ROOT="${OUTPUT_ROOT}",MECHANISM_DECISION="${MECHANISM_DECISION}",EXPECTED_CARD_SHA256="${EXPECTED_CARD_SHA256}",EXPECTED_SOURCE_MANIFEST_SHA256="${EXPECTED_SOURCE_MANIFEST_SHA256}",EXPECTED_MECHANISM_DECISION_SHA256="${EXPECTED_MECHANISM_DECISION_SHA256}",EXPECTED_PROFILE_DECISION_SHA256="${EXPECTED_PROFILE_DECISION_SHA256}",EXPECTED_DATASET_MANIFEST_SHA256="${EXPECTED_DATASET_MANIFEST_SHA256}",EXPECTED_FIELD_MANIFEST_SHA256="${EXPECTED_FIELD_MANIFEST_SHA256}" "${REPO_ROOT}/scripts/neurips_2026/allen_cahn_mechanistic_bridge/run_reduce.sh"
