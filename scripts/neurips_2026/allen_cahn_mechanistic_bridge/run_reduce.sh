#!/usr/bin/env bash
#SBATCH --job-name=ac-bridge-reduce
...
: "${EXPECTED_FIELD_MANIFEST_SHA256:?missing EXPECTED_FIELD_MANIFEST_SHA256}"
: "${EXPECTED_PROFILE_DECISION_SHA256:?missing EXPECTED_PROFILE_DECISION_SHA256}"

cd "${REPO_ROOT}"
uv run python -m experiments.neurips_2026.allen_cahn_mechanistic_bridge.reduce_label_aware \
  --field_manifest "${OUTPUT_ROOT}/field_manifest.json" \
  --expected_field_manifest_sha256 "${EXPECTED_FIELD_MANIFEST_SHA256}" \
