#!/bin/bash
#
# Queue Duffing LISTA L=8 vs L=10 LR sweep plus collection.
#
# Submit:
#   sbatch scripts/queue_duffing_lista_seq_lr.sh
#
# Optional overrides:
#   BASE_OUT=/network/scratch/l/lia/skae/duffing_lista_seq_lr_20260303
#   OUT_DIR=results/duffing_lista_seq_lr_20260303
#   NUM_STEPS=20000 EVAL_PROFILE=full
#
#SBATCH --job-name=queue_duf_slr
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --mem=1G
#SBATCH --time=00:10:00
#SBATCH -o /network/scratch/l/lia/skae/queue-duf-slr-%j.out
...
echo "Queueing Duffing LISTA Seq LR Sweep"
echo "Timestamp: $(date)"
...
SWEEP_JOB_ID=$(sbatch \
  ...
  "${ROOT_DIR}/scripts/sweep_duffing_lista_seq_lr.sh" | awk '{print $4}')

COLLECT_JOB_ID=$(sbatch \
  --dependency="afterany:${SWEEP_JOB_ID}" \
  --export=ALL,BASE_OUT="${BASE_OUT}",OUT_DIR="${OUT_DIR}" \
  "${ROOT_DIR}/scripts/collect_duffing_lista_seq_lr.sh" | awk '{print $4}')
