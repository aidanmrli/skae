#!/bin/bash
#
# Execute one research-paper benchmark task-table row inside an allocation.
#
# Required env vars:
#   TASK_TSV=<path>
...
#   ARRAY_OFFSET=0
#
# This payload has no SBATCH directives. Submit run_paper_benchmark_array.sh
# for one row per array element, or run_paper_benchmark_packed_array.sh for
# several rows per allocated GPU.
