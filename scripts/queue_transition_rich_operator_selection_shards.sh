#!/bin/bash
#
# Submit one operator-selection shard per root label, then a merge job.
#
# Required env vars:
#   ROWS_CSVS=<comma-separated forecasting_rows.csv files>
#   OUT_DIR=<final operator-selection output directory>
...
if (( ${#ROOT_LABELS[@]} == 0 )); then
  echo "No root labels resolved for operator-selection sharding." >&2
  exit 1
fi
...
MERGE_JOB_ID=$(
  ...
  SEEDS_CSV="${SEEDS_CSV}" \
    sbatch \
      ...
      scripts/merge_transition_rich_operator_selection_shards.sh | awk '{print $4}'
)
...
printf 'LOG_DIR=%q\n' "${LOG_DIR}"
printf 'OPERATOR_SELECTION_ROWS_CSV=%q\n' "${OUT_DIR}/operator_selection_rows.csv"
