...
EXPERIMENT_TAG="${EXPERIMENT_TAG:-dense_lista_easy_parity_stage1_${DATE_TAG}}"
PHASE_LABEL="${PHASE_LABEL:-stage1}"
...
TASK_TSV="${TASK_DIR}/dense_lista_easy_parity_stage1.tsv"
MANIFEST_JSON="${TASK_DIR}/dense_lista_easy_parity_stage1_manifest.json"
ROOT_SPECS_FILE="${ROOT_SPEC_DIR}/dense_lista_easy_parity_stage1_roots.txt"
...
if [[ -n "${NUM_STEPS_CSV:-}" ]]; then
  BUILD_ARGS+=(--num_steps_csv "${NUM_STEPS_CSV}")
fi
if [[ -n "${LR_PAIRS_CSV:-}" ]]; then
  BUILD_ARGS+=(--lr_pairs_csv "${LR_PAIRS_CSV}")
fi
if [[ -n "${WEIGHT_DECAY:-}" ]]; then
  BUILD_ARGS+=(--weight_decay "${WEIGHT_DECAY}")
fi

uv run python tools/build_dense_lista_easy_parity_tasks.py "${BUILD_ARGS[@]}"
...
echo "Queued dense LISTA easy-system parity stage-1 sweep."
