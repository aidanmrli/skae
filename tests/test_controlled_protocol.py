...
def test_manifest_is_exactly_the_frozen_paper_matrix():
    ...
    assert CONTROLLED_NUM_STEPS == 200_000
    assert CONTROLLED_BATCH_SIZE == 256
    assert CONTROLLED_TARGET_SIZE == 256
    assert CONTROLLED_SEQUENCE_LENGTH == 8
    assert tuple(CONTROLLED_SEEDS) == tuple(range(15))


def test_retained_recipes_preserve_reported_architecture_differences():
    dense_lista = get_controlled_model(
        "lista_dense_signsplit_p256_hardinit_basin_partition"
    )
    block_lista = get_controlled_model(
        "lista_blockdiag_signsplit_hardinit_basin_partition"
    )
    soft_lista = get_controlled_model(
        "lista_dense_softblock_signsplit_p256_hardinit_basin_partition"
    )
    dense_mlp = get_controlled_model(
...
def test_system_metadata_and_default_dt_remain_available_to_evaluators():
    assert get_controlled_basin_count("gated_local_linear") == 3
    assert resolve_controlled_default_dt("claude:cal_pentagon_5") == 0.03
    with pytest.raises(KeyError, match="Unknown controlled paper system"):
        get_controlled_basin_count("claude:checkerboard_potential")


def test_manifest_jsonable_has_no_exploratory_rows():
    payload = controlled_manifest_jsonable()
