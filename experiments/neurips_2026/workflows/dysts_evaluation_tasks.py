...
def _is_run_dir(path: Path, *, require_training_receipt: bool = False) -> bool:
    return (
        path.is_dir()
        and (path / "checkpoint.pt").exists()
        and (
            not require_training_receipt
            or (path / "training_success.json").exists()
        )
    )


def _latest_seed_run(
    system_dir: Path,
    seed: int,
    *,
    require_training_receipt: bool = False,
) -> Path | None:
    run_dirs: List[Path] = []
    for seed_dir in sorted(system_dir.glob(f"**/seed_{seed}")):
        ...
        for child in sorted(seed_dir.iterdir()):
            if _is_run_dir(child, require_training_receipt=require_training_receipt):
                run_dirs.append(child)
        if _is_run_dir(seed_dir, require_training_receipt=require_training_receipt):
...
def _read_root_specs(path: Path, *, allow_root_subset: bool = False) -> List[RootSpec]:
    rows: List[RootSpec] = []
    ...
    if allow_root_subset:
        if not labels or len(labels) != len(set(labels)):
            raise ValueError(f"Root labels must be a nonempty unique subset; got {labels}")
        unknown = tuple(label for label in labels if label not in DYSTS_MODEL_ROW_IDS)
        if unknown:
            raise ValueError(f"Unknown Dysts paper root labels: {unknown}")
        expected_order = tuple(label for label in DYSTS_MODEL_ROW_IDS if label in labels)
        if labels != expected_order:
            raise ValueError(
                "Subset root labels must retain paper-protocol order; "
                f"expected {expected_order}, got {labels}"
            )
    elif labels != DYSTS_MODEL_ROW_IDS:
        raise ValueError(
...
def _require_complete_coverage(
    *,
    ...
    """Fail when a sealed evaluation packet omits any expected training run."""

    if len(rows) != expected_rows or missing:
        raise RuntimeError(
            "Incomplete Dysts evaluation roster: "
            f"found {len(rows)}/{expected_rows} receipt-backed runs "
            f"with {len(missing)} missing entries."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Require training_success.json beside every checkpoint.",
    )
    parser.add_argument(
        "--require_complete",
        action="store_true",
        help="Fail unless every requested root/system/seed run is present.",
    )
    parser.add_argument(
        "--allow_root_subset",
        action="store_true",
        help="Allow an ordered nonempty subset of paper roots for a focused replacement row.",
    )
...
def main() -> None:
    args = parse_args()
    root_specs = _read_root_specs(
        Path(args.root_specs_tsv),
        allow_root_subset=bool(args.allow_root_subset),
    )
    systems = (
    ...
    rows, missing = _build_rows(
        ...
        require_training_receipt=bool(args.require_training_receipt),
    )
